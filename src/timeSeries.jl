...
function fixSlashesInColumeNames!(df::DataFrame)
    rename!(n -> replace(string(n), "/"=>"_"), df)
end
...
function pipeline(df::DataFrame, threshold, level)
	...
	stack(_, Not(presentFixedColumns), variable_name=:DateS, 
				value_name=:Value) |>  # make date columns into entries (wide to narrow)

	@transform(_, Date=Date.(String.(:DateS), "m_d_y") .+ Dates.Year(2000)) |>  # Make real date types

	groupby(_, vcat(presentLevels, "Date")) |> # Aggregate out to the level we want (e.g. :Country_Region)
	combine(_, "Value" => sum) |> 
	
	@where(_, :Value_sum .>= threshold) |>  # Start where the value is at or over threshold

	@orderby(_, :Date) |>    # Sort by date - earliest date for that threshold should be first for that country

	## Determine the days since the first "event"
	groupby(_, presentLevels) |>
	combine([:Date, :Value_sum] => (d, v) -> (
			daysSince = Dates.value.(d .- d[1]),  # We want an Int, not a date period type
			Date=d,
			Value=v,
			Value_new=v.- lag(v, default=0)  # Get the number of new deaths per day
		), _) |> 

	## Add the rolling 7 day means
	groupby(_, presentLevels) |> 
	combine(AsTable([:Date, :daysSince, :Value, :Value_new]) => x -> (
			Date = x.Date,
	...
	@where(_, :daysSince .> 0)  # Remove the zero (first) day since the running average doesn't work for that day
